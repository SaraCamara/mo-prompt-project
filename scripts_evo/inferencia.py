...
def run_scenario(model, strategy, initial_prompts, df, top_k, num_epochs, log_path):
    ...
    for prompt in initial_prompts:
        acc, f1 = evaluate_prompt(prompt, model, strategy, df)
    ...
    for epoch in range(num_epochs):
        ...
        try:
            new_prompt = generate_prompt_with_gpt4o([(p, f1) for p, _, f1 in top_prompts])
            print(f"Novo prompt gerado: {new_prompt}")
        except Exception as e:
            print(f"Erro ao gerar prompt com GPT-4o Mini: {e}")
            break

        acc, f1 = evaluate_prompt(new_prompt, model, strategy, df)
        print(f"Desempenho → Accuracy: {acc:.3f}, F1: {f1:.3f}")
        results.append((new_prompt, acc, f1))
...
def main():
    if len(sys.argv) < 3:
        print("Uso: python scripts/inferencia.py <modelo> <caminho_dataset_csv>")
        sys.exit(1)
